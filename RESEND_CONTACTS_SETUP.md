...
- An existing contact marked by this petition as created **without opting in** (`aa_created_no_updates=yes`) is globally re-subscribed when they later explicitly check the updates box. Their opt-in segment membership is added, so the new preference takes effect. If a contact previously opted in and later unsubscribed, or was created elsewhere and is already unsubscribed, this form does **not** override that earlier unsubscribe. Use a separate confirmed re-subscription/preference workflow for those cases, including older no-opt-in contacts created before this marker existed.
